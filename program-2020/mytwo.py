...
def cnnet(input_patch_img, label, num_patch, is_training):
...
def live_traindata():
    ...
    filenames = [os.path.join(SIQAD_ready.DATA_DIR, 'image_' + str(i) + '.tfrecords')
                 for i in ORDER[0:SIQAD_ready.TRAIN_DATA_NUM]]#遍历i经前式得的每个值组成此list
    for f in filenames:
...
def live_testdata():
    # Input images and labels.
    filenames = [os.path.join(SIQAD_ready.DATA_DIR, 'image_' + str(i) + '.tfrecords')
                 for i in ORDER[SIQAD_ready.TRAIN_DATA_NUM: SIQAD_ready.TRAIN_DATA_NUM + SIQAD_ready.TEST_DATA_NUM]]
    for f in filenames:
        if not tf.gfile.Exists(f):
            raise ValueError('Failed to find file: ' + f)
    return filenames
...
def evaluate(type):
    ...
    with graph.as_default() as g_assessment:
        ...
        if type == 'test':
            ...
            for i in range(SIQAD_ready.TEST_DATA_NUM):
                num_pic_eva += SIQAD_ready.DISNUM_PER_IMAGE
        else :
            filenames = live_traindata()
            for i in range(SIQAD_ready.TRAIN_DATA_NUM):
                num_pic_eva += SIQAD_ready.DISNUM_PER_IMAGE
        patch_img, label, num_patch = SIQAD_ready.distored_input(filenames, seed, type="test")  # float32
        scores, loss_n = cnnet(patch_img, label, num_patch, is_training=False)
        # Restore the moving average version of the learned variables for eval.
        # variable_averages = tf.train.ExponentialMovingAverage(
        #     SIQAD_ready.MOVING_AVERAGE_DECAY)
        # variables_to_restore = variable_averages.variables_to_restore()
        # saver = tf.train.Saver(variables_to_restore)
        saver = tf.train.Saver()
    with tf.Session(graph=graph) as sess:
        if type == 'test':
            checkpoint_file = os.path.join(SIQAD_ready.TEMP_MODEL, 'temp_model.ckpt')
        else:
            # checkpoint_file = os.path.join(SIQAD_ready.TEMP_bestMODEL+ "model"+ str(seed)+"/", 'best_model.ckpt')
            checkpoint_file = os.path.join(SIQAD_ready.TEMP_MODEL, 'temp_model.ckpt')
...
def train():
    ...
    with graph.as_default():
        num_picture = 0
        for i in range(SIQAD_ready.TRAIN_DATA_NUM):
            num_picture += SIQAD_ready.DISNUM_PER_IMAGE
        max_step = int(SIQAD_ready.NUM_EPOCH * num_picture / SIQAD_ready.BATCH_SIZE)
        print(max_step)  # 打印图片输进网络的总次数
        iters_per_epoch = num_picture//SIQAD_ready.BATCH_SIZE
        ...
        patch_img, label, num_patch = SIQAD_ready.distored_input(filenames_train, seed, type="train")
        scores, loss_n = cnnet(patch_img, label, num_patch, is_training=True)
...
if __name__ == '__main__':
    ORDER = np.random.permutation(SIQAD_ready.TRAIN_DATA_NUM + SIQAD_ready.TEST_DATA_NUM)  # 原图像随机排序
