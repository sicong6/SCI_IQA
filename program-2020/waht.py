...
def cnnet(input_patch_img, label, is_training):
    ...
    return score, loss, label  # output,loss
    ...
    with graph.as_default() as g_assessment:
        ...
        if type == 'test':
            ...
            for i in range(SIQAD_ready.TRAIN_DATA_NUM):
                num_pic_eva += SIQAD_ready.DISNUM_PER_IMAGE
        patch_img, label = SIQAD_ready.distored_input(filenames, seed, type="test")  # float32
...
def train():
    ...
    with graph.as_default():
        ...
        patch_img, label = SIQAD_ready.distored_input(filenames_train, seed, type="train")
        scores, loss_n, lab = cnnet(patch_img, label, is_training=True)
    ...
    with tf.Session(graph=graph) as sess:
        ...
        sre, srr, la = sess.run([loss_n, scores, lab])
        print(la)
        print(srr.shape)
