...
def distored_input(filenames,seed,type="train"):
    ...
    with tf.variable_scope("input_data"):
        ...
        img, label, type_ref = iterator.get_next()
        type_ref_int = tf.to_int32(type_ref, name='ToInt32')
        num = NUM_PATCHES[type_ref_int[0]]
        patch_img = tf.reshape(img, [BATCH_SIZE*num, PATCH_SIZE, PATCH_SIZE, DEPTH])
        return patch_img, label, num#, type
