...
def cnnet(input_patch_img, label, is_training):
    with arg_scope([layers.conv2d], padding='SAME',
        ...
        # net_qua = layers.fully_connected(feature_con, 512)
        # net_qua = layers.fully_connected(net_qua, 256)
        # output_extra = layers.fully_connected(net_qua, 1, activation_fn=None)
        # label2 = tf.log(label / output)
        # loss_ex = tf.losses.absolute_difference(label2, output_extra)  # + reg
        # loss = loss + loss_ex
        # output_ex = output * tf.exp(output_extra)
...
def train():
    ...
    with graph.as_default():
        ...
        print(max_step)  # 打印64patch输进网络的总次�?
...
if __name__ == '__main__':
    ...
    for i in range(1):
        ORDER = np.random.permutation(SIQAD_ready_unite_random.TRAIN_DATA_NUM + SIQAD_ready_unite_random.TEST_DATA_NUM)  # 原图像随机排�?
        # ORDER = ORDER_list[1]
        ...
        best_srocc, best_plcc, best_epoch = evaluate('train')  # test->temp   train->best
